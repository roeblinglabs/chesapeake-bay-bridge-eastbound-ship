...
from vessel_analysis import (analyze_vessel, CHESAPEAKE_BAY_BRIDGE_EASTBOUND_PIERS, BRIDGE_LAT, BRIDGE_LON,
...
with title_col:
    st.title("🌉 Chesapeake Bay Bridge Eastbound Vessel Collision Monitoring System")
...
with col1:
    ...
    # Create map centered on Chesapeake Bay Bridge
    m = folium.Map(
    ...
    # Add Chesapeake Bay Bridge marker (BLUE - infrastructure)
    folium.Marker(
        [BRIDGE_LAT, BRIDGE_LON],
        popup="Chesapeake Bay Bridge Eastbound",
        tooltip="Chesapeake Bay Bridge Eastbound",
...
with col2:
    ...
    with st.expander("View Details", expanded=False):
        ...
        st.markdown("""
        • **Name:** Chesapeake Bay Bridge (William Preston Lane Jr. Memorial Bridge)
        • **Location:** Maryland, connecting Sandy Point to Kent Island (38.99°N, 76.38°W)
        • **Year Built:** 1952 (Original eastbound span)
        • **Parallel Span Added:** 1973 (Westbound span)
        • **Owner:** Maryland Transportation Authority
        • **Bridge Type:** Steel cantilever through truss with suspension spans
        """)

        st.markdown("### Physical Characteristics")
        st.markdown("""
        • **Main Span Length:** 1,600 feet (suspension section)
        • **Total Bridge Length:** 4.3 miles (6.9 km)
        • **Deck Width:** 28 feet (2 lanes per span)
        • **Vertical Clearance:** 186 feet MHW (at main span)
        • **Number of Piers:** 20 (eastbound span)
        """)

        st.markdown("### Traffic & Importance")
        st.markdown("""
        • **Average Daily Traffic:** ~27,000 vehicles (eastbound)
        • **Annual Traffic:** ~10 million vehicles (both spans)
        • **Functional Classification:** Principal arterial (US Route 50/301)
        """)
    ...
    with st.expander("View Details", expanded=False):
        ...
        st.markdown("""
        • **Chart Reference:** NOAA Chart 12273 (Chesapeake Bay)
        • **Channel:** Main shipping channel to Baltimore
        • **Maintained Depth:** 50 feet (main channel)
        • **Project Depth:** Varies by location
        """)

        st.markdown("### Clearances")
        st.markdown("""
        • **Vertical Clearance:** 186 feet MHW (main span between towers)
        • **Horizontal Clearance:** 1,500 feet (main navigation span)
        """)

        st.markdown("### Currents & Tides")
        st.markdown("""
        • **Maximum Current:** ~1-2 knots (tidal)
        • **Tidal Range:** ~1-2 feet (Mean Range)
        • **Current Pattern:** Influenced by Chesapeake Bay tidal flow
        """)

        st.markdown("### Vessel Traffic")
        st.markdown("""
        • **Annual Transits:** Significant commercial traffic to Baltimore
        • **Typical Vessels:** Container ships, tankers, bulk carriers, recreational
        • **VTS Coverage:** Coast Guard Sector Maryland-NCR
        """)

        st.markdown("### Pier-Specific Data")
        st.markdown("""
        • **Number of Piers:** 20 (eastbound span)
        • **Main Towers:** Pier 9 and Pier 10
        • **Anchorages:** Pier 7 and Pier 12
        • **Water Depth at Main Channel:** ~100 feet
        ...
        st.markdown("""
        *📊 Data sources:*
        - *Navigation: NOAA Nautical Chart 12273*
        - *Pier capacity: Placeholder - requires structural engineering analysis*
        - *Pier coordinates: Provided by user*
